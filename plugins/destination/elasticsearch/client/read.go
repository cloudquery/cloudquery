...
func (*Client) createResultsArray(table *schema.Table) []elasticsearch.Value {
	results := make([]elasticsearch.Value, 0, len(table.Columns))
...
func (c *Client) Read(ctx context.Context, table *schema.Table, sourceName string, res chan<- []any) error {
	...
	q.Parameters = []elasticsearch.QueryParameter{
		{
			Name:  "cq_source_name",
			Value: sourceName,
		},
