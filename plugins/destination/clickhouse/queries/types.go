...
func chStructType(_struct *arrow.StructType) (string, error) {
	...
	fields, err := chFieldsDefinitions(_struct.Fields())
	if err != nil {
		return "", err
	}
...
func chFieldsDefinitions(fields []arrow.Field) ([]string, error) {
	res := make([]string, len(fields))
	for i, field := range fields {
		fieldDef, err := chFieldDefinition(field)
		if err != nil {
			return nil, err
		}
		res[i] = fieldDef
...
func chFieldDefinition(field arrow.Field) (string, error) {
	fieldType, err := chFieldType(field)
	if err != nil {
		return "", err
	}
	return sanitizeID(field.Name) + " " + fieldType, err
}
