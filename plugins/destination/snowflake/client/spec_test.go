...
func TestSpec_Config(t *testing.T) {
	tests := []struct {
		...
		{
			Spec: client.Spec{
				ConnectionString: "user:pass@account/database?warehouse=user_warehouse&schema=public",
			},
			WantDSN: "user:pass@account.snowflakecomputing.com:443?account=account&database=database&ocspFailOpen=true&schema=public&validateDefaultParameters=true&warehouse=user_warehouse",
		},
		{
			Spec: client.Spec{
