	...
	return &schema.Table{
		...
		Columns: []schema.Column{
			...
			},
			{
				...
				Type:        schema.TypeJSON,
				Resolver:    resolveAppsDaemonSetsOwnerReferences,
			},
			{
				Name:        "finalizers",
			...
			},
			{
				...
				Type:        schema.TypeJSON,
				Resolver:    resolveAppsDaemonSetsManagedFields,
			},
			{
			...
			},
			{
				...
				Type:        schema.TypeJSON,
				Resolver:    resolveAppsDaemonSetsTemplate,
			},
			{
...
func resolveAppsDaemonSetsOwnerReferences(ctx context.Context, meta schema.ClientMeta, resource *schema.Resource, c schema.Column) error {
...
func resolveAppsDaemonSetsManagedFields(ctx context.Context, meta schema.ClientMeta, resource *schema.Resource, c schema.Column) error {
	p, ok := resource.Item.(appsv1.DaemonSet)
	if !ok {
		return fmt.Errorf("not a appsv1.DaemonSet instance: %T", resource.Item)
	}
...
func resolveAppsDaemonSetsTemplate(ctx context.Context, meta schema.ClientMeta, resource *schema.Resource, c schema.Column) error {
	p, ok := resource.Item.(appsv1.DaemonSet)
	if !ok {
		return fmt.Errorf("not a appsv1.DaemonSet instance: %T", resource.Item)
	}
