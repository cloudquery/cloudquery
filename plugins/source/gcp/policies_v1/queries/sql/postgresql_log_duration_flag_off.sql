...
SELECT gsi.name                                                                                        AS resource_id,
       ...
       'Ensure "log_duration" database flag for Cloud SQL PostgreSQL instance is set to "on" (Manual)' AS title,
       ...
       CASE
           ...
           ELSE 'pass'
           END                                                                                         AS status
FROM gcp_sql_instances gsi LEFT JOIN JSONB_ARRAY_ELEMENTS(gsi.settings->'databaseFlags') AS f ON f->>'name'='log_duration';
