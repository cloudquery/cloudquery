...
SELECT gsi.name                                                                                             AS resource_id,
       ...
       'Ensure "remote access" database flag for Cloud SQL SQL Server instance is set to "off" (Automated)' AS title,
       ...
       CASE
           ...
           ELSE 'pass'
           END                                                                                              AS status
FROM gcp_sql_instances gsi LEFT JOIN JSONB_ARRAY_ELEMENTS(gsi.settings->'databaseFlags') AS f ON f->>'name'='remote access';
