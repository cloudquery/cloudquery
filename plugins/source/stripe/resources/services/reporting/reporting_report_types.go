...
func ReportingReportTypes() *schema.Table {
	return &schema.Table{
		...
		Transform:   transformers.TransformWithStruct(&stripe.ReportingReportType{}, transformers.WithSkipFields("APIResource", "ID")),
...
func fetchReportingReportTypes(ctx context.Context, meta schema.ClientMeta, parent *schema.Resource, res chan<- any) error {
	...
	it := cl.Services.ReportingReportTypes.List(&stripe.ReportingReportTypeListParams{})
	for it.Next() {
