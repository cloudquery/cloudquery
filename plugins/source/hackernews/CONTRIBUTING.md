# HackerNews Source Plugin Contribution Guide

Thanks for contributing to CloudQuery! You are awesome. This document serves as a guide for adding new services and resources to the HackerNews source plugin.
...
Every supported HackerNews service has a recipe file under [codegen/recipes](codegen/recipes). For example, all service API resources are listed in [codegen/recipes/services.go](codegen/recipes/services.go). 
...
A few important things to note when adding functions that call the HackerNews API:

- If possible, always use an API call that allows you to fetch many resources at once
- Take pagination into account. Use `Paginator`s if the HackerNews service supports it. Ensure you fetch **all** the resources.
