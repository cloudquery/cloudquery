WITH
    ds AS (
        SELECT resource_id as logic_workflow_id, jsonb_array_elements(diagnostic_settings_resource -> 'properties' -> 'logs') AS logs FROM azure_monitor_diagnostic_settings
    ),
    details AS (
