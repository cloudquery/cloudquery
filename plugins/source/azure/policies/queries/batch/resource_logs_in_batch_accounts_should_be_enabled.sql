WITH settings_with_logs
         AS (SELECT resource_id, diagnostic_settings_resource -> 'properties' ->> 'storageAccountId' as storage_account_id, JSONB_ARRAY_ELEMENTS(diagnostic_settings_resource -> 'properties' -> 'logs') AS logs
...
insert
into azure_policy_results
SELECT :'execution_time', :'framework', :'check_id', 'Resource logs in Batch accounts should be enabled',
       subscription_id,
       id,
       case
