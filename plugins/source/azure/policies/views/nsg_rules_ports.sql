...
WITH tmp AS (
    SELECT subscription_id, id, name, rls FROM azure_network_security_groups, json_array_elements(security_rules) AS rls
),
ansg AS (
    SELECT tmp.*, dprs FROM tmp, json_array_elements_text(rls->'destinationPortRanges') AS dprs
)
(
SELECT
