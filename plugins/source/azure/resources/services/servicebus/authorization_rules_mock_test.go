...
func buildAuthorizationRules(t *testing.T, ctrl *gomock.Controller, c *client.Services) {
	if c.Servicebus == nil {
	...
	var response api.TopicsClientListAuthorizationRulesResponse
	require.NoError(t, faker.FakeObject(&response))
	// Use correct Azure ID format
	const id = "/subscriptions/test/resourceGroups/test/providers/test/test/test"
	response.Value[0].ID = to.Ptr(id)

	mockTopicsClient.EXPECT().NewListAuthorizationRulesPager(gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any()).
		Return(client.CreatePager(response)).MinTimes(1)

	buildAccessKeys(t, ctrl, c)
}
