...
func SubAssessments() *schema.Table {
	return &schema.Table{
		...
		Description:          "https://learn.microsoft.com/en-us/rest/api/defenderforcloud/sub-assessments/list-all?tabs=HTTP#securitysubassessment",
...
func fetchSubAssessments(ctx context.Context, meta schema.ClientMeta, parent *schema.Resource, res chan<- any) error {
	...
	pager := svc.NewListAllPager("/subscriptions/"+cl.SubscriptionId, nil)
	for pager.More() {
