...
func fetchContacts(ctx context.Context, meta schema.ClientMeta, parent *schema.Resource, res chan<- any) error {
	...
	if cqClient.IsIncrementalSync() {
		return syncIncrementally(ctx, cqClient, res, "contacts", "lastmodifieddate")
	}

	return syncAllContacts(ctx, cqClient, res)
}

func syncAllContacts(ctx context.Context, cqClient *client.Client, res chan<- any) error {
