...
func appAssesments() *schema.Table {
	return &schema.Table{
		...
		Transform: transformers.TransformWithStruct(&types.AppAssessment{},
			transformers.WithNameTransformer(transformARN("AssessmentArn")),
			transformers.WithPrimaryKeys("AssessmentArn")),
		Multiplex: client.ServiceAccountRegionMultiplexer("resiliencehub"),
		Columns:   []schema.Column{client.DefaultAccountIDColumn(true), client.DefaultRegionColumn(true), appARNTop},
