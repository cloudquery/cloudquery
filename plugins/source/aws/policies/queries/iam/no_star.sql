...
with pvs as (
    select id, (cq_urldecode(v->>'Document'))::jsonb as doc from aws_iam_policies, jsonb_array_elements(aws_iam_policies.policy_version_list) AS v
), violations as (
    select
        id,
        COUNT(*) as violations
    from pvs,
        JSONB_ARRAY_ELEMENTS(
            case JSONB_TYPEOF(doc->'Statement')
                when 'string' then JSONB_BUILD_ARRAY(doc->>'Statement')
                when 'array' then doc->'Statement'
            end
        ) as statement,
