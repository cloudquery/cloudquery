...
func buildBackupVaultsRecoveryPoints(t *testing.T, ctrl *gomock.Controller) client.Services {
	mock := mocks.NewMockBackupClient(ctrl)

	item := types.RecoveryPointByBackupVault{}

	err := faker.FakeData(&item)
	if err != nil {
		t.Fatal(err)
	}
	mock.EXPECT().ListRecoveryPointsByBackupVault(gomock.Any(), gomock.Any(), gomock.Any()).Return(

		&backup.ListRecoveryPointsByBackupVaultOutput{
			RecoveryPoints: []types.RecoveryPointByBackupVault{item},
		}, nil)
