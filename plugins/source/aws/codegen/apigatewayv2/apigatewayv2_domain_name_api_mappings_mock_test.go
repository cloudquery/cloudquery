...
func buildApigatewayv2DomainNameApiMappings(t *testing.T, ctrl *gomock.Controller) client.Services {
	mock := mocks.NewMockApigatewayv2Client(ctrl)

	item := types.ApiMapping{}

	err := faker.FakeData(&item)
	if err != nil {
		t.Fatal(err)
	}
	mock.EXPECT().GetApiMappings(gomock.Any(), gomock.Any(), gomock.Any()).Return(

		&apigatewayv2.GetApiMappingsOutput{
			Items: []types.ApiMapping{item},
		}, nil)
