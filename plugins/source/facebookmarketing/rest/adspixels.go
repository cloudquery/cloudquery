...
type AdspixelssResponseStruct struct {
	Data   []Adspixel `json:"data"`
	Paging *Paging    `json:"paging"`
}

func (facebookClient *FacebookClient) ListAdspixels(ctx context.Context, page string) (items []Adspixel, nextPage string, err error) {
	query := url.Values{}

	query.Set("fields", strings.Join(getAllFieldJsonTags(Adspixel{}), ","))
	...
	path, err := url.JoinPath("v16.0", "act_"+facebookClient.AdAccountId, "adspixels")
	if err != nil {
		return nil, "", err
	}
	...
	var responseStruct AdspixelssResponseStruct
