-- https://hub.steampipe.io/mods/turbot/kubernetes_compliance/queries/daemonset_container_privilege_disabled
...
select uid                              AS resource_id,
      ...
      CASE WHEN
          (SELECT COUNT(*) FROM daemonset_containers 
            WHERE daemonset_containers.uid = k8s_apps_daemon_sets.uid 
            AND daemonset_containers.container->'securityContext'->>'privileged' = 'true') > 0
          THEN 'fail'
          ELSE 'pass'
          END                          AS status
FROM k8s_apps_daemon_sets;
