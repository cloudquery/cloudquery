...
func ValidatingWebhookConfigurations() *schema.Table {
	return &schema.Table{
		...
		Transform: client.TransformWithStruct(&v1.ValidatingWebhookConfiguration{}, transformers.WithPrimaryKeys("UID")),
...
func fetchValidatingWebhookConfigurations(ctx context.Context, meta schema.ClientMeta, parent *schema.Resource, res chan<- any) error {
	cl := meta.(*client.Client).CoreAPI().AdmissionregistrationV1().ValidatingWebhookConfigurations()
