...
func fetchReviewApps(ctx context.Context, meta schema.ClientMeta, _ *schema.Resource, res chan<- any) error {
	...
	for _, it := range items {
		...
		for nextRange.Max != 0 {
			ctxWithRange := context.WithValue(ctx, "nextRange", nextRange) // nolint:revive,staticcheck
			v, err := c.Heroku.ReviewAppList(ctxWithRange, it.ID, nextRange)
			if err != nil {
				return errors.WithStack(err)
			}
			res <- v
