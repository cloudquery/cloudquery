		...
		return diag.Diagnostics{
			RedactError(accounts, diag.NewBaseError(err,
				...
				append(opts,
					...
					diag.WithSeverity(diag.WARNING),
					ParseSummaryMessage(err),
					diag.WithDetails("CloudQuery AWS provider has been throttled. This is unexpected - you can open an issue on github (https://github.com/cloudquery/cloudquery/plugins/aws/issues) or contact us on discord (https://cloudquery.io/discord)"),
				)...),
			),
		}
	}
