...
resource "aws_instance" "elbv1-instance-2" {
...
resource "aws_elb" "bar" {
  ...
  instances = [
    aws_instance.elbv1-instance-2.id,
...
resource "aws_load_balancer_policy" "policy-ssl" {
  load_balancer_name = aws_elb.bar.name
  policy_name = "policy${var.test_prefix}${var.test_suffix}"
