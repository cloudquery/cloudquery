...
resource "aws_ecs_task_definition" "aws_ecs_clusters_task_definition" {
  ...
  memory = "2048"
  cpu = "1024"
  requires_compatibilities = ["EC2"]                                                              # TASK running role
}
...
resource "aws_ecs_service" "aws_ecs_clusters_service" {
  ...
  network_configuration {
    security_groups = [aws_security_group.aws_ecs_clusters_security_group.id]
    subnets = [
      aws_subnet.aws_vpc_subnet.id,
      aws_subnet.aws_vpc_subnet2.id]
    assign_public_ip = "false"
  }
  depends_on = [aws_lb_target_group.aws_elbv2_lb_target_group]
}
