...
data "aws_iam_policy_document" "secretsmanager_secret_iam_policy" {
  statement {
    effect  = "Allow"
...
resource "aws_lambda_permission" "secretsmanager_secret_lambda_permission" {
  function_name = aws_lambda_function.secretsmanager_secret_rotation_function.function_name
...
resource "aws_lambda_function" "secretsmanager_secret_rotation_function" {
  ...
  runtime          = "python3.9"
  timeout          = 30
  role             = aws_iam_role.secretsmanager_secret_iam_role.arn
...
resource "aws_secretsmanager_secret_rotation" "secretsmanager_secret_rotation" {
  ...
  rotation_lambda_arn = aws_lambda_function.secretsmanager_secret_rotation_function.arn
...
data "archive_file" "rotation_lambda_func_zip_inline" {
  type        = "zip"
  output_path = "./tmp/rotation_lambda_zip_inline.zip"
  source {
    content  = <<EOF
