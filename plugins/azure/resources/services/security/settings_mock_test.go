...
func buildSecuritySettingsMock(s security.BasicSetting) func(*testing.T, *gomock.Controller) services.Services {
...
func TestSecuritySettings(t *testing.T) {
	t.Run("data_export_settings", func(t *testing.T) {
		...
		id := client.FakeResourceGroup + "/" + *setting.ID
		setting.ID = &id
		client.AzureMockTestHelper(t, SecuritySettings(), buildSecuritySettingsMock(setting), client.TestOptions{})
	})
}
