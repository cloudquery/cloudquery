...
`)
	...
	genCmd := &cobra.Command{
		...
		Long:  "Write configuration to drift.hcl in current directory",
		Run: func(cmd *cobra.Command, args []string) {
			ret = d.genConfig(ctx, req)
		},
	}
...
func (d *DriftImpl) genConfig(ctx context.Context, req *model.ExecuteRequest) *model.ExecutionResult {
	configPath := d.ID() + ".hcl"

	fs := afero.NewOsFs()
	if info, _ := fs.Stat(configPath); info != nil {
		return &model.ExecutionResult{
			Error: fmt.Errorf("Config file %s already exists", configPath),
		}
	}

	f := hclwrite.NewEmptyFile()
	rootBody := f.Body()
	blk := hclwrite.NewBlock("module", []string{d.ID()})
	gohcl.EncodeIntoBody(d.hclConfig, blk.Body()) // FIXME doesn't work because gohcl doesn't (and shouldn't) understand hcl.Body
	rootBody.AppendBlock(blk)

	var buffer bytes.Buffer
	buffer.WriteString("// Configuration AutoGenerated by CloudQuery CLI\n")
	if n, err := buffer.Write(f.Bytes()); n != len(f.Bytes()) || err != nil {
		return &model.ExecutionResult{Error: fmt.Errorf("failed: %w", err)}
	}
	...
	return &model.ExecutionResult{
		Results: []string{
			fmt.Sprintf("configuration generated successfully to %s", configPath),
		},
	}
}
