...
func (h TableCreator) CreateTable(ctx context.Context, conn *pgxpool.Conn, t, p *schema.Table) error {
	...
	if _, err := conn.Exec(ctx, fmt.Sprintf(dropTableView, t.Name)); err != nil {
		return fmt.Errorf("failed to drop view for table: %s: %w", t.Name, err)
	}

	if _, err := conn.Exec(ctx, fmt.Sprintf(createTableView, t.Name)); err != nil {
		return fmt.Errorf("failed to create view for table: %s: %w", t.Name, err)
	}
