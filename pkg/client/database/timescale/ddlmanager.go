...
const (
	listHyperTables = `SELECT hypertable_name FROM timescaledb_information.hypertables WHERE hypertable_schema=$1 AND hypertable_name LIKE 'aws_directconnect_%' ORDER BY 1`
...
func (h DDLManager) SetupHistory(ctx context.Context) error {
	var tables []string
	if err := pgxscan.Select(ctx, h.pool, &tables, listHyperTables, history.SchemaName); err != nil {
		return fmt.Errorf("failed to list hypertables: %w", err)
	}

	for _, table := range tables {
		if err := h.configureHyperTable(ctx, table); err != nil {
			return fmt.Errorf("failed to configure hypertable for table: %s: %w", table, err)
		}
		if err := h.recreateView(ctx, table); err != nil {
...
func (h DDLManager) configureHyperTable(ctx context.Context, tableName string) error {
	tName := fmt.Sprintf(`"%s"."%s"`, history.SchemaName, tableName)

	if _, err := h.pool.Exec(ctx, fmt.Sprintf(setChunkTimeInterval, h.cfg.TimeInterval), tName); err != nil {
		return err
	}
	...
	if _, err := h.pool.Exec(ctx, fmt.Sprintf(dataRetentionPolicy, h.cfg.Retention), tName); err != nil {
		return err
	}
...
func (h DDLManager) recreateView(ctx context.Context, table string) error {
	if err := h.pool.BeginTxFunc(ctx, pgx.TxOptions{}, func(tx pgx.Tx) error {
...
func DropViews(ctx context.Context, conn *pgxpool.Pool) error {
	var tables []string
...
func AddHistoryFunctions(ctx context.Context, conn *pgxpool.Pool) error {
