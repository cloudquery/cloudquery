...
func TestExecutor_ExecutePolicies(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			ErrorOutput:   "broken_policy_query: ERROR: syntax error at or near \"SECT\" (SQLSTATE 42601)",
			ShouldBeEmpty: true,
			Pass:          true,
		},
		{
			...
			ErrorOutput:   "broken_policy_view: ERROR: syntax error at or near \"TCELES\" (SQLSTATE 42601)",
			ShouldBeEmpty: true,
			Pass:          true,
		},
	}
