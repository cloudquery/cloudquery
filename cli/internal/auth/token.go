...
func GetAuthTokenIfNeeded(logger zerolog.Logger, sources []*specs.Source, destinations []*specs.Destination) (string, error) {
	...
	if err != nil {
		recommendLogin := strings.HasPrefix(err.Error(), "Hint:")
		if recommendLogin {
			logger.Warn().Err(err).Msg("when using the CloudQuery registry, it's recommend to login first via `cloudquery login`. Logging in allows for better rate limits and downloading of premium plugins")
			return "", nil
		}
