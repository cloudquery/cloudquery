...
func syncConnection(ctx context.Context, pm *plugin.PluginManager, specReader *specs.SpecReader, sourceSpec specs.SourceSpec) error {
	...
	if err != nil {
		return fmt.Errorf("fetch: failed to configure source plugin %s: %w", sourceSpec.Name, err)
	}
	if validationResult != nil && !validationResult.Valid() {
		for _, err := range validationResult.Errors() {
			fmt.Printf("Source spec error at field: %s, %s\n", err.Field(), err.Description())
		}
	...
	for i, destination := range sourceSpec.Destinations {
		...
		if err := destPlugins[i].GetClient().Migrate(ctx, sourceSpec.Name, sourceSpec.Version, tables); err != nil {
