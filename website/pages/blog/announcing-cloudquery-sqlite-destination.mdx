...
Today I'm excited to announce the release of our new SQLite destination plugin, so you can now sync all supported CloudQuery [source plugins](https://hub.cloudquery.io/plugins/source) to a [SQLite](https://www.sqlite.org/index.html) database.

Check out [our docs](https://hub.cloudquery.io/plugins/destination/cloudquery/sqlite) to see how to configure the SQLite destination plugin.
...
If you don't want to run a database or docker, running SQLite as destination will be super performant similar to our [File destination plugin](https://hub.cloudquery.io/plugins/destination/cloudquery/file) but with the ability to do complex queries and store everything in one file.
