...
- [Azure Source Plugin](https://hub.cloudquery.io/plugins/source/cloudquery/azure)
- [PostgreSQL Destination Plugin](https://hub.cloudquery.io/plugins/destination/cloudquery/postgresql)
...
Before going off and writing queries, we need to sync the data from Azure to our PostgreSQL database. To do that, we'll use the `cloudquery sync` command. Let's start with creating a configuration file (For the full configuration reference, check out the [Azure Source Plugin](https://hub.cloudquery.io/plugins/source/cloudquery/azure/configuration) and [PostgreSQL Destination Plugin](https://hub.cloudquery.io/plugins/destination/cloudquery/postgresql)):
