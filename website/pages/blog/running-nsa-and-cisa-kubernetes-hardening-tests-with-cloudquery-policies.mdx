...
Following is a quick start to run the policy. Otherwise, checkout full details on our [docs](https://hub.cloudquery.io/addons/transformation).

### Prerequisite

Please follow the [quickstart guide](/docs/quickstart) documentation on how to `install`, `init`, and `fetch` the [K8S Provider](https://hub.cloudquery.io/plugins/source).
