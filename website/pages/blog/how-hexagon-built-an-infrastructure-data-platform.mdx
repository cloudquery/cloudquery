...
With these stages complete, the data is queryable from Athena, either by table or through the resources view. The tables and views can also be loaded into QuickSight (via data set), where graphs and dashboards can be created.
...
Pushing from the beginning for a fully serverless solution will bring lots of benefits in the long run. There is no need for time-consuming or costly updates and virtually zero maintenance. The cloud provider and CloudQuery updates and maintains the whole solution making it extremely attractive. This brings to the Hexagon CCoE team important peace of mind since they highly value the zero maintenance model.
