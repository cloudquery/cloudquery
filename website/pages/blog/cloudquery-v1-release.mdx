...
For this New official destination plugins support [auto migrations](/docs/advanced-topics/migrations) and [release stages](https://hub.cloudquery.io/plugins/source)
...
As we are a big believer in data-engineering best practices we provide a set of standard [SQL queries](https://hub.cloudquery.io/addons/transformation) for popular benchmarks. Most importantly, they are all open source and not abstracted behind any custom policy language so you can re-use them, customize and apply to your needs while enjoying the SQL eco-system.
