...
Additionally, depending on the use case and service for delegated administrator, deregistering or changing the delegated administrator account may either need to be done from the management account or the delegated administrator account.
...
* How to change or deregister delegated administrator accounts and where the call must be made from.
...
By leveraging multiple delegated administrator accounts, usage of the organization management account can be limited to infrequent activites and break-glass access.  This reduces the possibility of misconfiguration of the management account, an account with a large blast radius of the rest of the accounts in the AWS organization.
...
There are activites that delegated administrators cannot do and thus will still require usage of the organization management account.
...
The service specific actions do make child calls to `organizations:RegisterDelegatedAdministrator` and other dependent actions.  The below screenshot shows the assocaited calls for `fms:AssociateAdminAccount`, `guardduty:EnableOrganizationAdminAccount`, `macie2:EnableOrganizationAdminAccount`, and `inspector2:EnableDelegatedAdminAccount`.
...
Lastly, we have Delegation via Organization Resource-Based Delegation Policies.  This recent release 
...
While deregistering a delegated administrator should be an infrequent action, we found knowing how to deregister a delegated administrator.  Use cases for deregistering delegated administrator accounts include deleting a delegated administrator if a service is no longer needed.  In some cases, certain services only allow for 1 delegated administrator and deregistering the initial delegated administrator is necessary prior to setting up a new delegated administrator.

Similar to registering delegated administrators, some services have service-specific actions for deregistering delegated administrator accounts.  

| AWS Service | Deregister Action | Management or Delegated Admin Account | 
| -------- | ------- | -------- |
...
| AWS EC2 (IPAM) | ec2:DisableIpamOrganizationAdminAccount | - |
...
| AWS Trusted Advisor | organizations:DeregisterDelegatedAdministrator | - |
| AWS CloudFormation StackSets | organizations:DeregisterDelegatedAdministrator | - |
| AWS Systems Manager Explorer | organizations:DeregisterDelegatedAdministrator | - |
| AWS Systems Manager Change Manager | organizations:DeregisterDelegatedAdministrator | - |
| AWS Account Management | organizations:DeregisterDelegatedAdministrator | - |
| AWS Backup | organizations:DeregisterDelegatedAdministrator | - |
| AWS Compute Optimizer | organizations:DeregisterDelegatedAdministrator | - |
| AWS Config | organizations:DeregisterDelegatedAdministrator | - |
| AWS DevOps Guru | organizations:DeregisterDelegatedAdministrator | - |
| AWS IAM Access Analyzer | organizations:DeregisterDelegatedAdministrator | - |
| AWS License Manager | organizations:DeregisterDelegatedAdministrator | - |
| AWS Network Manager | organizations:DeregisterDelegatedAdministrator | - |
| AWS S3 Storage Lens | organizations:DeregisterDelegatedAdministrator | - |
| AWS Service Catalog | organizations:DeregisterDelegatedAdministrator | - |
| AWS IAM Identity Center | organizations:DeregisterDelegatedAdministrator | - |
| Reachability Analyzer | organizations:DeregisterDelegatedAdministrator | - |
