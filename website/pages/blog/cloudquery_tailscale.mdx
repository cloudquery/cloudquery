...
date: 2023/02/07
description: >-
  This tutorial will show you the basic of ingesting your Tailscale configuration to any database/datalake with CloudQuery
author: yevgenypats
---
...
Tailscale source plugin for CloudQuery is now officially maintained by Tailscale - Big thanks for the Tailscale team!.
...
so you can have all your infrastructure data in one place and corrolate between different services.
...
First let's quickly look at the source configuration so the plugin can authenticate with the new OAuth Client
...
Find all devices that weren't seen more than 30 days:
...
Find all devices with key expiary disabled
...
Other cool things once you have Tailscale configuration in your database/datalake is that you can join and query it with other CloudQuery [source plugins](https://www.cloudquery.io/docs/plugins/sources/overview).
...
You can slice and dice tailscale config data in many ways but this hopefully gives you a great starting point!