...
Adopting Apache Arrow as the CloudQuery in-memory type system enables us to gain better performance, data interoperability and developer experience. Some plugins that are going to gain an immediate boost of rich type systems are our db->db replication plugins such as [PostgreSQL CDC](https://hub.cloudquery.io/plugins/source/cloudquery/postgresql) source plugin (and all [database destinations](/docs/plugins/destinations/overview)) that are going to get support for all available types including nested ones.
