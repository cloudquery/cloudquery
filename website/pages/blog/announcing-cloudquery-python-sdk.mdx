...
The scheduler's responsibility is to manage the order in which tables are synced, ensuring that tables are synced in the correct order to avoid dependency issues, and to place limits on the number of concurrent requests and memory usage. For the scheduler in the Python SDK, we implemented a new queue-based scheduling algorithm that can take advantage of multiple cores, without plugin developers needing to think about this.
...
If you're keen to dive right in and develop a source plugin in Python, check out the [Python SDK documentation](https://github.com/cloudquery/plugin-sdk-python) and sample plugins on GitHub:
