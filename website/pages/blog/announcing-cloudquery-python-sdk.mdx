...
CloudQuery has a pluggable architecture and uses [Apache Arrow](https://arrow.apache.org/) over [gRPC](https://grpc.io/) for communication between plugins. Source and destination plugins are independent of one another, and this architecture allows plugins to be written in different languages but still communicate with one another. Until now, we've only provided an SDK for writing plugins in Go, but we're excited to announce that we now have an SDK for writing plugins in Python as well!

![cloudquery high-level architecture](/images/cloudquery-architecture.png)

## Features

The Python SDK provides a number of features to make plugin development easier, and plugins written in Python performant.
...
`MyPlugin` is a class that extends `cloudquery.sdk.plugin.Plugin`.

### Base Plugin Class

A CloudQuery Python plugin, like `MyPlugin` above, should subclass `cloudquery.sdk.plugin.Plugin` and only needs to implement three methods: `init`, `get_tables` and `sync`. The `init` method is called when the plugin is started, and is where you can do any initialization work. The `get_tables` method should return a list of tables that the plugin supports. The `sync` method is called when a table needs to be synced, and is where the SDK scheduler can be used to manage the syncing of all the supported tables.

### Multi-threaded Scheduler

The scheduler is an optional (but highly recommended) component of a plugin, with the responsibility to manage the order in which tables are synced, to ensure that tables are synced in the correct order to avoid dependency issues, and place limits on the number of concurrent requests and memory usage. For the scheduler in the Python SDK, we implemented a new queue-based scheduling algorithm that can take advantage of multiple cores, without plugin developers needing to think about this.

The `sync` method of a plugin needs to pass a list of its tables and options to the scheduler, and the scheduler will take care of the rest. Here is an example of this code [from the Typeform plugin](https://github.com/cloudquery/cloudquery/blob/main/plugins/source/typeform/plugin/plugin.py):
...
To support cross-platform packaging of Python plugins (and other languages in the future) in `v3.12.0` we introduced a new `docker` registry type to the CloudQuery CLI. Whereas Go-based plugins are downloaded as binaries from GitHub releases, Python plugins can be downloaded as Docker images from the specified Docker registry. This allows us to support multiple platforms, and also makes it easier to distribute plugins that have dependencies on external libraries.
