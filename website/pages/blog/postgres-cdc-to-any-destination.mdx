...
of the supported [destinations](/docs/plugins/destinations/overview), without the need for Kafka or any intermediary (You can still stream to Kafka or Google PubSub).
This gives a much simpler setup to operate by just running one CloudQuery binary!

In the following section, we will go through how our CloudQuery PostgreSQL source plugin works internally. (For running and configuring, refer to the [docs](https://www.cloudquery.io/docs/quickstart/macOS).)
