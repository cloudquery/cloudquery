...
One of the most powerful features of PostgreSQL is its ability to replicate data, and in this article, we'll take a deep dive into the topic of logical replication.

## What is CDC?

[CDC (Change-Data-Capture)](https://en.wikipedia.org/wiki/Change_data_capture) is a term used in a set of design patterns in databases where one can track changes to the data and act upon it.
In PostgreSQL, CDC can be achieved via a feature called logical replication.
...
whereas logical replication only replicates changes to the data, such as the insertions, updates, and deletions of rows in tables.
Logical replication provides a more flexible and efficient way to replicate data as it only replicates the changes in the data,
...
CloudQuery PostgreSQL source plugin gives you the ability to sync data/changes in real time from a PostgreSQL database, directly to any
of the supported [destinations](https://www.cloudquery.io/docs/plugins/destinations/overview), with the need for Kafka or any intermediary (You can still stream to Kafka or Google PubSub).
This gives a much simpler setup to operate by just running one CloudQuery binary!

In this section, we will go through how our CloudQuery PostgreSQL source plugin works internally. (For running and configuration, refer to the docs.)

### Set `wal_level` to logical

To use logical replication, you will need to make sure you started your database with `wal_level`.

See our docs on how to set `wal_level` to logical in various environments.
