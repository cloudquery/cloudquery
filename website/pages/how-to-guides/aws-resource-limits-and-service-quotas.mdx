...
Now, we can look for additional use cases such as API limits.  For this query, we'll need to sync `aws_cloudtrail_events` as a resource from AWS.  This resource can take time to sync due to the sheer amount of data in []`aws_cloudtrail_events`](https://www.cloudquery.io/docs/plugins/sources/aws/tables/aws_cloudtrail_events).
