---
title: Docker - Read-only Containers
description: Learn how to run containers with read-only file system.
---

# Docker - Read-only Containers

By default, CloudQuery requires a read-write file system to create a socket file used by the plugins to communicate with the server. It is however not possible to create a read-write socket on a read-only file system. For this to work, a plugin needs to be started as a gRPC server locally.
...
spec:
  ...
  spec:
    connection_string: "postgresql://cq:password123@host.docker.internal:5432/cq-test?sslmode=disable"
```
...
We will use the `ghcr.io/cloudquery/cloudquery:latest` container as a base and add all files in it. Then we will override the entry point with our start script:
