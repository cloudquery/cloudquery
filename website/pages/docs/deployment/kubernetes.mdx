...
Due to its flexibility and standardization across all cloud providers, Kubernetes is the #1 way that DevOps & Platform Engineers prefer to deploy workloads and microservices.
...
In this example we're using the DigitalOcean source plugin and the Postgres destination plugin. But you can find out more about building config files and integrations [here](https://www.cloudquery.io/integrations).
...
In this tutorial, we have seen how to set up CloudQuery on Kubernetes and sync data to a PostgreSQL database. If you have any questions, check out the video above or join our [Discord community](https://cloudquery.io/discord) to chat with us!
