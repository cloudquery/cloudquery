...
id: cloudquery_sync
namespace: dev
description: Caching CloudQuery plugins
...
The flow will extract all tables from the HackerNews API and load them to a destination PostgreSQL database. You may notice that the input to the `config_files` task is virtually identical to your CloudQuery configuration file.
