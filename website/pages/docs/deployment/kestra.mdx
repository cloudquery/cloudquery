...
This tutorial shows how to run CloudQuery as a [Kestra](https://kestra.io/) flow, using the the HackerNews API as a source and CSV file as a destination. Kestra is an open-source event-driven orchestrator, which has a [dedicated CloudQuery plugin](https://kestra.io/plugins/plugin-cloudquery).
...
- run your syncs based on events from other systems e.g. a new file in a folder, new message in a queue, or a new webhook request
...
Inside the Kestra UI, go to the `Flows` tab and click on `Create`. You can now paste the following content:
