...
1. Back in the SSH session connected to the VM instance, run the following command to start the sync:
   ...
   ```
2. Hopefully, you should see a successful sync! If not, check `cloudquery.log` for errors.
...
4. Save the file and exit the editor. The sync will now run every day at 1am.
