...
If you are using a database other than PostgreSQL, you will need to modify the `destination` section of the config file accordingly. See the [destinations](https://hub.cloudquery.io/plugins/destination) section of the docs for more information.

Similarly, if you are using a different source, follow the [documentation for your source plugin](https://hub.cloudquery.io/plugins/source) to get the correct config.
