...
For ingestion, you need to use an ELT (Extract-Load-Transform) tool that supports the [sources](/docs/plugins/sources/overview) and [destinations](https://hub.cloudquery.io/plugins/destination) that you need. CloudQuery (as you are on the CloudQuery website :) ) is an open-source, high-performance ELT framework that supports more than 40 sources and 15 destinations with great coverage for cloud infrastructure APIs.
