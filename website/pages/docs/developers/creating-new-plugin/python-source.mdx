---
title: Creating a New CloudQuery Source Plugin in Python
description: Learn how to write a new CloudQuery source plugin from scratch in Python.
...
Every table will typically have its own `.py` file inside the plugin `tables` directory.
...
As the Python SDK is still new, we don't yet have a plugin scaffold generator. For now, we recommend starting by copying the code from an existing Python plugin. Two reference plugins are [Typeform](https://github.com/cloudquery/cloudquery/tree/main/plugins/source/typeform) and [Square](https://github.com/cloudquery/cloudquery/tree/main/plugins/source/square). The Typeform plugin is relatively simple, and the Square plugin provides an example of generating tables from an OpenAPI spec.
...
You can also build a Docker container for the plugin, and then either run it directly as a gRPC server or via the `docker` registry in a config file. See an [example Docker file for a Python plugin here](https://github.com/cloudquery/cloudquery/blob/main/plugins/source/typeform/Dockerfile).
...
- The [Python Source Plugin Template](https://github.com/cloudquery/python-plugin-template/) is an easy-to-use template for building your own plugin.
- The [Python XKCD Example](https://github.com/cloudquery/python-plugin-template/tree/XKCD-Example) is an example of using the Python Source Plugin Template to build a simple plugin.
- The [Typeform Source Plugin](https://github.com/cloudquery/cloudquery/tree/main/plugins/source/typeform) is an example of a simple plugin that fetches data from a REST API.
- The [Square Source Plugin](https://github.com/cloudquery/cloudquery/tree/main/plugins/source/square) is an example of a medium-complexity plugin that fetches data from a REST API using an OpenAPI spec.

## Resources

- [CloudQuery Community](https://community.cloudquery.io)
- [Python Plugin Template](https://github.com/cloudquery/python-plugin-template)
- [Python SDK Source Code](https://github.com/cloudquery/plugin-sdk-python)
- [CloudQuery Plugin SDK on PyPI](https://pypi.org/project/cloudquery-plugin-sdk/)
