---
title: Creating a New CloudQuery Source Plugin in Go
...
This guide will help you develop a new source or destination plugin for CloudQuery in Go. CloudQuery's modular architecture means that a source plugin can be used to fetch data from any third-party API, and then be combined with a destination plugin to insert data into any supported destination. We will cover the basics of how to get started, and then dive into some more advanced topics. We will also cover how to release your plugin for use by the wider CloudQuery community.
...
Here are the basic steps of adding support for incremental tables to your plugin:

1. Within the plugin client initialize the state client using `state.NewConnectedClient` or `state.NewConnectedClientWithOptions` ([see example in our Hacker News plugin](https://github.com/cloudquery/cloudquery/blob/main/plugins/source/hackernews/resources/plugin/client.go)).
2. **You must** call `Close()` on the returned client.
3. When defining tables add `IsIncremental: true` to the table definition. Add `IncrementalKey: true` to the column definition for the column that is used as the cursor ([see example](https://github.com/cloudquery/cloudquery/blob/main/plugins/source/hackernews/resources/services/items/items.go)).
4. In your table resolver manipulate the cursor using the `stateClient.GetKey()` and `stateClient.SetKey()` methods.
5. **You must** call `stateClient.Flush()` in the plugin client after the sync runs (see the Hacker News example above). If you don't this then the new **cursor will not be persisted** to the state backend.
...
Visit the [Publishing a Plugin to the Hub](/docs/developers/publishing-a-plugin-to-the-hub) guide for instructions on how to publish your plugin to the CloudQuery Hub.
