...
3. When defining tables add `IsIncremental: true` to the table definition and `IncrementalKey: true` to the column definition ([see example](https://github.com/cloudquery/cloudquery/blob/main/plugins/source/hackernews/resources/services/items/items.go)).
