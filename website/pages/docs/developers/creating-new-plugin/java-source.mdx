...
There are two options for running a plugin before as a developer before it is released: as a gRPC server, or as a docker container. We will briefly summarize both options here, or you can read about them in more detail in [Running Locally](/docs/developers/running-locally).
