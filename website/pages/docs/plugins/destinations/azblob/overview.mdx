...
The Azure Blob destination utilizes batching, and supports `batch_size`, `batch_size_bytes` and `batch_timeout_ms` options (see below).
...
- `batch_timeout_ms` (integer, optional. default: 30000 (30 seconds))

  Inactivity time in milliseconds before starting a new object.
