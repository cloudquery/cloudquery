...
```
### migrate to latest versions of the plugins.

By default, the `sync` command executes a migration before syncing resources. However, there are situations where you might need to run it in forced mode for migration.

Note: Before proceeding with the migration, it is crucial to take a backup of the database to ensure that there is no impact on the production environment.
