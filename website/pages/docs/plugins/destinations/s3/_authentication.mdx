...
If you are using a custom S3 endpoint, you can specify it using the `AWS_S3_ENDPOINT` environment variable:

```bash copy
export AWS_S3_ENDPOINT=https://my-custom-s3-endpoint.com
```

If you're using authentication, the `region` option in the spec determines the signing region used.
