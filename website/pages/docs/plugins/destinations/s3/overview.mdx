...
- `no_test` (boolean) (optional, default `false`)

  At its default value of `false`, a test object is generated/written on each sync to ensure write access to the given bucket and path. If you are sure that the bucket and path are writable, you can set this to `true` to skip the test.
