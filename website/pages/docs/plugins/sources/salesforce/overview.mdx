...
- `api_version` (string array, optional):
    
  Mandatory API version to be used. Check [this article](https://help.salesforce.com/s/articleView?id=000386929&type=1) to identify the API version for your org.
...
- `oauth_url` (string array, optional):
    
  By default the plugin will use the standard production OAuth endpoint (https://login.salesforce.com/services/oauth2/token). Alternatively, sandbox OAuth endpoint can be configured (https://test.salesforce.com/services/oauth2/token)
