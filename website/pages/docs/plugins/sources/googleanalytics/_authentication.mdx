...
The suggested way is to use [Workload identity federation](https://cloud.google.com/iam/docs/workload-identity-federation). If not available, you can use service account keys and export the location of the key via `GOOGLE_APPLICATION_CREDENTIALS`. **This is not recommended as long-lived keys present a security risk**.
