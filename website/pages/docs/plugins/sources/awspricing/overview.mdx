...
- `region_codes` ([]string) (default: empty. Will sync data for all regions)
  ...
  Note: This is not just AWS Commercial regions. It includes Wavelength Zones and and regions in other partitions (Gov Cloud and China). 

- `offer_codes` ([]string) (default: empty. Will sync data for all services)

  Specify which services to sync data for. Regular expressions are supported, matches must be an exact match
