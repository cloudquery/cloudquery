...
<Callout type="info">
The `table_options` is an Enterprise only feature
</Callout>
...
- **enterprise version only** `streaming_sync` (`array`) (default: empty)
...
- `start_time` (timestamp) 
  ...
  Defines the place in the stream where record processing should begin. By default, the time at which the the sync began will be used.

- `streaming_sync_only` (`bool`) (default: `false`)

  By default CQ will do a full sync on the specified tables before starting to consume the events in the stream. This parameter enables users to skip the pull based sync and go straight to the streaming sync.
