...
1. CloudQuery is expecting data in a single sharded Kinesis stream. The format of the data needs to be in the same form as CloudWatch Logs uses to send CloudTrail Events to a Kinesis Data stream. CloudQuery maintains a reference solution that uses CloudFormation to deploy.
