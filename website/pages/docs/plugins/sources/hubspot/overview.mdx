...
- `table_options` ([map[string][TableOptions]](#table-options) spec, optional. Default: `empty`)
  ...
  Table Options for HubSpot entities that will be in sync.

### Table Options

- `table_name` ([TableOptionsSpec](#table-options-spec) spec, optional. Default: `empty`)

  Table name to define the options when collecting data from the entity on HubSpot.
