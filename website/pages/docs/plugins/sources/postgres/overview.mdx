...
The CloudQuery Postgres plugin syncs your Postgres database to any of the supported CloudQuery destinations (e.g. PostgreSQL, Snowflake, BigQuery, …).
This plugins also supports CDC via Postgres logical replication which enables keeping your Postgres up to date with any destination just by subscrbing to changes.
...
- `cdc` (bool, optiona. Default: false)
