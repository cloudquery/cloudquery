...
```bash
cloudquery generate destination <YOUR_PLUGIN_NAME>
```
...
Also make sure for every source plugin to specify which destinations you want to sync to.

```bash
cloudquery generate source <YOUR_PLUGIN_NAME>
```

## Start Syncing

Now you can start syncing the data from your source plugins to the specified destination
