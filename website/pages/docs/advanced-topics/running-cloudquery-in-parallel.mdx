...
or when fetching from very large accounts. This page provides important guidelines for this use case.

<Callout> 

All guidelines in this page also apply when running a single cloudquery instance with multipe source-plugins.
...
can fetch resource `B` from project `1`, or resource `A` from proejct `2`, but can never fetch resource `A` from project `1`.
