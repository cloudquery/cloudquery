...
In order to resume from a previous position, incremental tables store some state, known as the **cursor**. When using the CloudQuery CLI, the cursor state is stored in a **backend**. Any destination plugin can be used as a backend. This state destination is often the same destination data is being written to for the sync, but it doesn't need to be. If the destination only supports `append` mode, each state update will generate a new row (with a new version) in the state table and may need to be cleaned up periodically. Destinations supporting `overwrite` mode don't have this issue.
