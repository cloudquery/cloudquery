...
Once you select the source plugin, its configuration UI will load. Notice the Setup Guide on the right side that will guide you through the invididual configuration options for the specific plugin.

[![Source Configuration](/images/docs/quickstart/source-2.png)](/images/docs/quickstart/source-2.png)

Some plugins support OAuth connection, other require an API key, or acess token. AWS plugin will help you deploy a CloudFormation stack to create an IAM role which will be used by CloudQuery to access the AWS data.
