...
Managed syncs are a great way to get started with CloudQuery and syncing data from source to destination without the need to deploy your infrastructure. You only need to select a source and destination plugin and CloudQuery will take care of the rest.
...
If you configure syncs to run Daily, they will run at midnight (UTC) every day. Weekly syncs will run at 12 AM UTC on Mondays, and monthly syncs will run at 12 AM UTC on the first day of each month.
...
- Docker-based cannot be used with Managed Syncs. This is a limitation during beta phase and will be removed before the general availability.
