...
description: Use this agent when the user wants to analyze or optimize CI pipeline times for a plugin, check a plugin's GitHub Actions workflow for best practices, or implement CI improvements. Input is the plugin name (e.g. "postgresql", "sqlite" for destination; "hackernews", "airtable" for source). This repo has mostly destination plugins.
