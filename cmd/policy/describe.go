...
func NewCmdPolicyDescribe(parentOptions policyOptions) *cobra.Command {
	cmd := &cobra.Command{
		Use:     "describe",
		Short:   describeShort,
		Long:    describeShort,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			c, err := console.CreateClient(cmd.Context(), util.GetConfigFile(parentOptions.Config), true, nil, util.InstanceId)
			if err != nil {
				return err
			}
