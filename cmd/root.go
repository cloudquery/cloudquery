...
func init() {
	...
	rootCmd.PersistentFlags().Bool("no-download", false, "disable plugins download. Will cause an error if a plugin is not found")
	...
	_ = viper.BindPFlag("no-download", rootCmd.PersistentFlags().Lookup("no-download"))
